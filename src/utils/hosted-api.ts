...
function getActionableErrorMessage(
  ...
  const baseMessages: Record<number, string> = {
    ...
    500: `Server error. This is temporary - try again shortly or report at github.com/FractionEstate/midnight-nextjs-mcp/issues`,
  ...
  const actionableMessage =
    baseMessages[status] ||
    `API error (${status}). Try again or report at github.com/FractionEstate/midnight-nextjs-mcp/issues`;
...
async function apiRequest<T>(
  ...
  if (lastError) {
    ...
    if (lastError.message.includes("github.com/FractionEstate")) {
      throw lastError;
    }
